...
def api_settings():
    ...
    while True:
        ...
        #keysAPI = open(".api/secrets.json", "r")
        ...
        if IntegrityAPI['message'] == "APIKEY_INVALID":
            ...
            time.sleep(1)
            print("Incorrect API key. Enigma will now close...")
            print("")
...
def market_settings():
    ...
    while True:
        ...
        print("")
        MARKET = input("Enter BTC market to trade e.g. 'BCC': ")
        MARKET = MARKET.upper()
...
while True:
    ...
    BTCBalanceArray = V1Bittrex.get_balance('BTC')
    BTCBalance = BTCBalanceArray['result']['Available']
    AltBalanceArray = V1Bittrex.get_balance(MARKET)
    ...
    print("Press 'b' to set buy order, or 's' to set sell order.")
    print("Press 'c' to cancel all open orders, or 'w' to check your BTC wallet.")
    ...
    if UserSelection == "1":
        ...
        print("")
        CustomOrder = input("Enter a bid price (e.g. 0.00002581): ")
        BalanceProg = input("Enter fraction of BTC balance to trade with (e.g. 0.10): ")

        BTCTrade = (float(BTCBalance) * float(BalanceProg) * 0.99) / float(CustomOrder)
        orderBuy = V1Bittrex.buy_limit(MarketString, BTCTrade, float(CustomOrder))

        print(orderBuy)

        print("")
        print("Ask order set at " + str(BTCTrade) + " " + str(MARKET) + " at " + CustomOrder + " BTC")
        print("")
        time.sleep(0.2)

    elif UserSelection == "S" or UserSelection == "s":
        print("")
        CustomOrder = input("Enter an ask price (e.g. 0.00002581): ")
        BalanceProg = input("Enter fraction of 'altcoin' balance to trade with (e.g. 0.10): ")

        AltTrade = float(AltBalance * int(BalanceProg))
        ...
        if OpenOrdersNumber > 0:
            ...
            for i in range(0, OpenOrdersNumber):
                ...
                OpenOrders.append(OpenOrdersIter)

            for j in range(0, len(OpenOrders) - 1):

                test = V1Bittrex.cancel(OpenOrders[j])
                print(test)
